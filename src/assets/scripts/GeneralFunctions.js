...
export const getData = async (base_url, url, type, dataParams = {}, isToken , isLoader = true, isHeaderJson = false, default_token = null) => {
    if (isLoader) {
        loader(true)
    }
    let header = {};
    if (isToken && default_token) {
        ...
        header = {headers: {Authorization: `Bearer ${isToken}`}};
        if (isHeaderJson) {
            header = {
                headers: {
                    "Content-Type": "application/json; charset=utf-8",
...
export const ChangeSwitchStatus = (e) => {
    let switch_value;
    if (parseInt(e) === 0) {
        switch_value = 1
        return switch_value
...
export const ChangeSwitchStatusOrdersOver = (e) => {
    let switch_value;
    if (parseInt(e) > -1) {
        switch_value = 1
        return switch_value
